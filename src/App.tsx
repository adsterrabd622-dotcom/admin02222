...
import { Video, LayoutGrid, Users, CreditCard, Settings } from 'lucide-react';
import { Avatar, AvatarFallback } from '@/components/ui/avatar';

export default function App() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'media' | 'users' | 'withdrawals' | 'settings'>('dashboard');
      ...
      <aside className="w-full md:w-64 bg-card border-b md:border-r flex flex-col p-4 md:p-6 transition-all duration-300 shrink-0">
        <div className="flex items-center justify-between md:justify-start gap-3 mb-4 md:mb-12">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-lg tracking-tight text-primary">Viral Earning</span>
          </div>
        ...
        <nav className="flex-1 overflow-x-auto md:overflow-visible">
          <ul className="flex md:flex-col gap-1 md:space-y-1">
            ...
            <li 
              ...
              className={`px-4 py-2 ${activeTab === 'users' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted/50'} rounded-md text-sm font-medium cursor-pointer flex items-center gap-3 whitespace-nowrap transition-colors`}
            >
              <Users className="w-4 h-4" /> Users List
            </li>
            <li 
              onClick={() => setActiveTab('withdrawals')}
              className={`px-4 py-2 ${activeTab === 'withdrawals' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted/50'} rounded-md text-sm font-medium cursor-pointer flex items-center gap-3 whitespace-nowrap transition-colors`}
            >
              <CreditCard className="w-4 h-4" /> Withdrawals
            </li>
            <li 
              onClick={() => setActiveTab('settings')}
              className={`px-4 py-2 ${activeTab === 'settings' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted/50'} rounded-md text-sm font-medium cursor-pointer flex items-center gap-3 whitespace-nowrap transition-colors`}
            >
              <Settings className="w-4 h-4" /> Earning Settings
            </li>
          </ul>
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0">
        <header className="h-14 md:h-16 border-b bg-card px-4 md:px-8 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            <h1 className="font-semibold text-[10px] md:text-sm uppercase tracking-widest text-muted-foreground">
              ...
              {activeTab === 'media' && 'Media Library Setup'}
              {activeTab === 'users' && 'User Management Terminal'}
              {activeTab === 'withdrawals' && 'Payout Processing Queue'}
              {activeTab === 'settings' && 'Application Configuration'}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Avatar className="h-8 w-8 md:h-9 md:w-9 border-2 border-background shadow-sm">
              <AvatarFallback className="bg-primary/20 text-primary uppercase text-xs">A</AvatarFallback>
            </Avatar>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-[1200px] mx-auto animate-in fade-in duration-700">
             {(activeTab === 'dashboard' || activeTab === 'media') && <Dashboard activeTab={activeTab} />}
        ...
        <footer className="h-10 md:h-12 border-t flex items-center justify-center px-8 bg-card/50 text-[10px] uppercase tracking-widest text-muted-foreground/30 shrink-0">
          &copy; {new Date().getFullYear()} Earning Admin &bull; Control Panel
        </footer>
      </div>
